...
def opt_ensemble(models, test_dataloader):
    with torch.no_grad():
        ...
        final_mask = np.zeros((len(test_dataloader),4, 224, 224), dtype=np.float32)
        ...
        for model in models:
            ...
            for idx, image in enumerate(tqdm(test_dataloader)):
                ...
                mask = torch.sigmoid(outputs).cpu().numpy()
                mask = np.squeeze(mask, axis=1)
                final_mask[idx] += mask / num_models
...
def main():
    ...
    #print(MODELNAME)
    ensemble_modelpath=['./stride112_hrnet_20.pt','./stride100_divied_deeplab_80.pt','./stride56_divided_unet_19.pt','./stride112_divided_deepUnet_40.pt']
...
#     optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    ...
    result=opt_ensemble(models, test_dataloader)
